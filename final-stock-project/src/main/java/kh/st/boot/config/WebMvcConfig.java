...
public class WebMvcConfig implements WebMvcConfigurer {
	...
	public WebMvcConfig(GuestInterceptor guestInterceptor, LoginInterceptor loginInterceptor,
	...
	public void addInterceptors(InterceptorRegistry registry) {
		...
		registry.addInterceptor(guestInterceptor).addPathPatterns("/login", "/join"); // 로그인, 회원가입 경로

		// LoginInterceptor 적용 경로 설정
		registry.addInterceptor(loginInterceptor).addPathPatterns("/login/process"); // 로그인 처리 경로

		// MemberInterceptor 적용 경로 설정
		registry.addInterceptor(memberInterceptor).addPathPatterns("/member/**") // 회원 관련 경로
